...
struct FSpeakerInfo
{
	...
	FSpeakerInfo()
	{
		...
		//DefaultSequenceSettings = FMovieSceneSequencePlaybackSettings();

		//FMovieSceneSequenceLoopCount InfiniteLooping;
		//InfiniteLooping.Value = -1;

		//DefaultSequenceSettings.bPauseAtEnd = true;
		//DefaultSequenceSettings.LoopCount = InfiniteLooping;
	}
	...
	//The shot to use when this speaker is talking
	UPROPERTY(EditAnywhere, Instanced, Category = "Camera")
	class UNarrativeDialogueShot* DefaultShot = nullptr;

	//Default sequence to play whilst this speaker is talking - overrides defaultshot
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Sequences")
	class ULevelSequence* DefaultSequence = nullptr;

	//Default playback settings for when this speakers shot is playing - removed because compile error in 5.1 even when including header/module
	//UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Sequences")
	//struct FMovieSceneSequencePlaybackSettings DefaultSequenceSettings;
...
public:
	...
	//Can this dialogue be exited by the player? In networked games, the server will check this to prevent clients exiting dialogue they shouldn't 
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Dialogue Details")
	bool bCanBeExited;

	//If selected, narrative will automatically rotate the OwningPawn and the NPCActor so that they face each other when the dialogue begins.
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Dialogue Details")
	...
	*/
	...
	//Any shots added to this array will randomly be cycled through by narratives dialogue cinecam.
	UPROPERTY(EditAnywhere, Instanced, Category = "Camera")
	TArray<class UNarrativeDialogueShot*> SelectReplyShots;

	//Any shots added to this array will randomly be cycled through by narratives dialogue cinecam.
	UPROPERTY(EditAnywhere, Instanced, Category = "Camera")
	TArray<class UNarrativeDialogueShot*> PlayerTalkingShots;

	//A sequence to play whilst the player is talking
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Sequences", meta=(AdvancedDisplay))
	class ULevelSequence* PlayerTalkingSequence;

	//Playback settings for when the player is talking
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Sequences", meta = (AdvancedDisplay))
	struct FMovieSceneSequencePlaybackSettings PlayerTalkingSequenceSettings;

	//A sequence to play whilst the NPC is waiting for the player to select their reply 
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Sequences", meta = (AdvancedDisplay))
	class ULevelSequence* SelectReplySequence;

	//Playback settings for when the NPC is Waiting for the player to select their reply 
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Sequences", meta = (AdvancedDisplay))
	struct FMovieSceneSequencePlaybackSettings SelectReplySequenceSettings;
...
protected:
	...
	*/
	...
	virtual void OnBeginDialogue_Implementation();
	...
	*/
	...
	virtual void PlayDialogueSequence(class ULevelSequence* Shot, const FMovieSceneSequencePlaybackSettings& Settings);
