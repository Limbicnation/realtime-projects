...
UDialogue::UDialogue()
{
	...
	PlayerTalkingSequenceSettings.bPauseAtEnd = true;
	PlayerTalkingSequenceSettings.LoopCount = InfiniteLooping;
	SelectReplySequenceSettings.bPauseAtEnd = true;
	SelectReplySequenceSettings.LoopCount = InfiniteLooping;
...
{	
	...
	if (Speakers.Num() == 0)
	{
		...
		DefaultSpeaker.DefaultSequence = nullptr;
...
void UDialogue::OnBeginDialogue_Implementation()
{

	/*It doesnt really make sense for the participants to not face each other, so do this automagically
	//TODO, cache these values and automatically rotate */
	if (NPCActor && OwningPawn)
	{
		if (bAutoRotateSpeakers)
		{
			const FVector Offset = (NPCActor->GetActorLocation() - OwningPawn->GetActorLocation()).GetSafeNormal();

			OldNPCRot = NPCActor->GetActorRotation();
			OldPlayerRot = OwningPawn->GetActorRotation();

			NPCActor->SetActorRotation((-Offset).Rotation());
			OwningPawn->SetActorRotation(Offset.Rotation());
		}

		if (bAutoStopMovement)
		{
			//The camera will line up its shot, but if the character is still moving the shot won't be lined up, so stop the characters from moving
	...
	if (bSpawnDialogueCamera && DialogueCameraClass && OwningController && OwningController->IsLocalPlayerController())
	{
...
void UDialogue::NPCFinishedTalking()
{
	...
	if (AvailableResponses.Num() && OwningComp && OwningComp->CurrentDialogue == this)
	{
		...
		if (SelectReplySequence)
		{
			PlayDialogueSequence(SelectReplySequence, SelectReplySequenceSettings);
		}
		else if (SelectReplyShots.Num() && DialogueCamera)
		{
			PlayDialogueShot(SelectReplyShots[FMath::RandRange(0, SelectReplyShots.Num() - 1)], OwningPawn, NPCActor);
		}
		else if (DialogueShots.Num())
		{
			PlayDialogueShot(DialogueShots[FMath::RandRange(0, DialogueShots.Num() - 1)], OwningPawn, NPCActor);
		}
...
void UDialogue::PlayDialogueShot(class UNarrativeDialogueShot* Shot, class AActor* Speaker, class AActor* Listener)
{
	...
	{
		DialogueCamera->SetActorTransform(Shot->GetShotTransform(this, Speaker, Listener));
		...
		{
			FVector FocusLoc = Speaker->GetActorLocation();
			FocusLoc.Z = CineCam->GetActorLocation().Z;
			CineCam->GetCineCameraComponent()->FocusSettings.ManualFocusDistance = FVector::Dist(CineCam->GetActorLocation(), FocusLoc);
		}
	}
}
...
{
	//By default, narrative will play the dialogues audio, any montages the NPC needs to play, and position the camera via a sequence or narrative dialogue shot, whatever is set. 

	PlayDialogueSound(LineToPlay);
	PlayDialogueAnimation(NPCReply, LineToPlay);

	//Check what the dialogue needs to do, with sequences taking precedence over shots, and dialogue lines taking precendes over speaker info
	if (LineToPlay.Sequence)
	{
		PlayDialogueSequence(LineToPlay.Sequence, LineToPlay.SequenceSettings);
	}
	else if (LineToPlay.Shot)
	{
		PlayDialogueShot(LineToPlay.Shot, NPCActor, OwningPawn);
	}
	else if (SpeakerInfo.DefaultSequence)
	{
		PlayDialogueSequence(SpeakerInfo.DefaultSequence, FMovieSceneSequencePlaybackSettings());
	}
	else if (SpeakerInfo.DefaultShot)
	{
		PlayDialogueShot(SpeakerInfo.DefaultShot, NPCActor, OwningPawn);
	}
	else if (DialogueShots.Num())
	{
		PlayDialogueShot(DialogueShots[FMath::RandRange(0, DialogueShots.Num() - 1)], NPCActor, OwningPawn);
	}
	else
	{
		StopDialogueSequence();
...
void UDialogue::PlayPlayerDialogue_Implementation(class UDialogueNode_Player* PlayerReply, const FDialogueLine& Line)
{
	...
	if (Line.Sequence)
	{
		PlayDialogueSequence(Line.Sequence, Line.SequenceSettings);
	}
	else if (Line.Shot)
	{
		PlayDialogueShot(Line.Shot, OwningPawn, NPCActor);
	}
	else if (PlayerTalkingSequence)
	{
		PlayDialogueSequence(PlayerTalkingSequence, PlayerTalkingSequenceSettings);
	}
	else if (PlayerTalkingShots.Num())
	{
		PlayDialogueShot(PlayerTalkingShots[FMath::RandRange(0, PlayerTalkingShots.Num() - 1)], OwningPawn, NPCActor);
	}
	else if (DialogueShots.Num())
	{
		PlayDialogueShot(DialogueShots[FMath::RandRange(0, DialogueShots.Num() - 1)], OwningPawn, NPCActor);
...
void UDialogue::PlayDialogueSequence(class ULevelSequence* Shot, const FMovieSceneSequencePlaybackSettings& Settings)
{
