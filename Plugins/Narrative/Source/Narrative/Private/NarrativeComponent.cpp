...
void UNarrativeComponent::BeginPlay()
{
	...
	if (HasAuthority() && !GetOwner()->bAlwaysRelevant)
	{
...
void UNarrativeComponent::QuestTaskProgressMade(const UQuest* Quest, const FQuestTask& ProgressedObjective, const class UQuestBranch* Step, int32 CurrentProgress, int32 RequiredProgress)
{
	if (Quest)
	{
		UE_LOG(LogNarrative, Log, TEXT("Quest %s made progress - task %s is now %d/%d"), *GetNameSafe(Quest), *(ProgressedObjective.TaskDescription.ToString()), CurrentProgress, RequiredProgress);
	}
}
